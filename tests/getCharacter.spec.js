...
describe('9 - Implemente os casos de teste da função `getCharacter`', () => {
  it('Verifica se a função `getCharacter` retorna o objeto do personagem corretamente.', () => {
    ...
    const brienneCharacter = getCharacter('Brienne');

    expect(getCharacter('Brienne')).toStrictEqual(brienneCharacter);

    // 4. Teste se a função retorna o objeto correto para o parâmetro 'Melissandre',
    const melissandreCharacter = getCharacter('Melissandre');

    expect(getCharacter('Melissandre')).toStrictEqual(melissandreCharacter)
    ...
    expect(getCharacter('White Walkers')).toBeUndefined();
  });
});
